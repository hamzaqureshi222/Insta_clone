...
class Firebase_Firestor {
  ...
  deletepost(postId){
    FirebaseFirestore.instance.collection('posts')
        .where("postId",isEqualTo: postId).get().then((querySnapshot) {
      for (var doc in querySnapshot.docs) {
        doc.reference.delete();
      }
    });
}
  Stream<int> commentsno(String postId) async* {
    try {
      ...
      for (var doc in querySnapshot.docs) {
        yield* doc.reference.collection('comments').snapshots().map((snapshot) {
          return snapshot.docs.length;
        });
      }
    } catch (error) {
      print("Error getting post document: $error");
      yield 0; // Return 0 if there's an error
    }
  }
